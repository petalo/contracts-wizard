/**
 * @fileoverview Template Processing System with Handlebars and Spanish Locale Support
 *
 ...
 *
 * Constants:
 * - renderer: Marked renderer configuration for HTML generation
 ...
 *
 * // TODO: Required modifications
 *
 * 1. Custom Business Requirements (No library support):
 *    - Missing Value Handling: Implement <span class="missing-value"> generation
 *    - CSV Data Integration: Process and inject CSV data into templates
 *    - Custom Logging: Maintain detailed logging for debugging
 *
 * 2. Library Configuration (Use existing features):
 *    - Marked:
 *      * Use built-in GFM (GitHub Flavored Markdown) for tables and lists
 *      * Enable HTML parsing with sanitization
 *      * Use native header level preservation
 *    - Handlebars:
 *      * Leverage built-in helpers (if, each, with)
 *      * Use native HTML escaping
 *      * Use default array indexing in #each
 *
 * 3. Integration Layer (Minimal custom code):
 *    - Bridge between Handlebars and Marked processing
 *    - Preserve HTML generated by our custom handlers
 *    - Maintain template processing order
 *
 * 4. Cleanup & Optimization:
 *    - Remove redundant custom implementations
 *    - Simplify renderer overrides
 *    - Use library defaults where possible
 *
 ...
 * @exports {Function} processMarkdownTemplate - Main template processor
 * @exports {Function} processEmptyValues - Empty value handler
 * @exports {Function} validateInputs - Input validator
 * @exports {Function} processDataFile - Data processor
 * @exports {Function} removeFrontmatter - YAML frontmatter cleaner
 * @exports {Function} preprocessImages - Image path converter to base64
 * @exports {Function} formatEmail - Email formatter to HTML link
 *
 * @example
 * // Process a markdown template with data
...
const customRenderer = require('../renderers/custom-text-renderer');

// Importar y registrar todos los helpers
...
// Assign custom renderer methods after creating the renderer
const renderer = new customRenderer();
Object.assign(marked.defaults.renderer, renderer);
 ...
 * @param {string} value - The placeholder value to display
...
/**
 * Register the 'with' helper for changing context
 * Allows accessing nested object properties without repeating the parent object name
 * Maintains the full path for proper data-field attributes
 *
 * @example
 * {{#with user}}
 *   Name: {{name}}
...
// Modify processEmptyValues function to handle nested paths
 ...
 * @return {Promise<void>}
 * @example
 * // Log details of generated files
 * await logGeneratedFiles('output/doc.html', 'output/doc.pdf', 'output/doc.md');
 */
 ...
 * @return {Promise<{data: Object, stats: Object}>} Processed data and statistics
 * @example
...
// Add function to extract fields from template
 ...
 *
 * Handles the complete lifecycle of document generation from markdown templates,
 * including data injection, format conversion, and output generation.
 *
 ...
 * @param {Object} [data] - Optional pre-processed data object
 * @returns {Promise<Object>} Generated file paths and processing statistics
 * @throws {AppError} When template processing fails
 * @example
 * // Basic usage with all files
 * const result = await processMarkdownTemplate(
 *   'template.md',
 *   'data.csv',
 *   'styles.css',
...
async function processMarkdownTemplate(
  ...
  try {
    ...
    const { data: rawData } = await processDataFile(dataPath, templateFields);
    ...
    // Log outer error details for debugging only
